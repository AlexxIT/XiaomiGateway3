...
def test_gateway_stats():
    ...
    assert p == {
        'network_pan_id': None, 'radio_channel': None, 'radio_tx_power': None
    }

    p = device.decode(GATEWAY, {
        'networkUp': True, 'networkPanId': '0x9180', 'radioTxPower': 7,
        'radioChannel': 15
    })
    assert p == {
        'network_pan_id': '0x9180', 'radio_tx_power': 7, 'radio_channel': 15
    }

    p = device.decode(GATEWAY, {
        'free_mem': 3488, 'ip': '192.168.1.123',
        'load_avg': '1.92|2.00|2.25|5/91|21135', 'rssi': 58,
        'run_time': 367357, 'setupcode': '123-45-678', 'ssid': 'WiFi',
        'tz': 'GMT3'
    })
    assert p == {
        'free_mem': 3488, 'load_avg': '1.92|2.00|2.25|5/91|21135', 'rssi': -42,
...
2: uart:16550A mmio:0x18147800 irq:47 tx:1846359 rx:3845724 oe:18 RTS|DTR"""
    ...
    assert p == {
        'bluetooth_tx': 19370, 'bluetooth_rx': 154557484, 'bluetooth_oe': 1684,
        'zigbee_tx': 1846359, 'zigbee_rx': 3845724, 'zigbee_oe': 18
    }
